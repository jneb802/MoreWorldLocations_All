...
public abstract class ItemData
{
    ...
    public ItemInfo Info => (info ?? constructingInfo).TryGetTarget(out ItemInfo itemInfo)
        ? itemInfo
...
public class ItemInfo : IEnumerable<ItemData>
{
    ...
    private static MethodInfo removeMethod = typeof(ItemInfo).GetMethods().Single(m =>
        m.Name == nameof(Remove) && m.IsGenericMethod && m.GetParameters()[0].ParameterType == typeof(string));
    ...
    static ItemInfo()
    {
        ...
        harmony.Patch(AccessTools.DeclaredMethod(typeof(ItemDrop), nameof(ItemDrop.Awake)),
            prefix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(ItemInfo), nameof(TrackAwakeningItem))),
            transpiler: new HarmonyMethod(
...
public static class ItemExtensions
{
    ...
    public static ForeignItemInfo? Data(this ItemDrop.ItemData item, string mod)
    {
        ...
        if (plugin.Instance.GetType().Assembly.GetType(className)?.GetMethod(nameof(Data),
        ...
        Debug.LogWarning(
            $"Mod {mod} has an {className} class, but no Data(ItemDrop.ItemData) method could be called on it.");
